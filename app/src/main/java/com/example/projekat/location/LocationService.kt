...
class LocationService : Service() {
    ...
    private val notifiedKladionice = mutableSetOf<String>()
    ...
    private fun start(
        ...
        locationClient.getLocationUpdates(1000L)
            ...
            .onEach { location ->
                ...
                if(nearby){
                    checkProximityToKladionice(location.latitude, location.longitude)
                }
    ...
    private fun createNotificationChannel() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description = "Obaveštavamo vas da se vaša lokacija prati u pozadini kako bi se proverilo da li ste u blizini neke kladionice"
            }
    ...
    private fun checkProximityToKladionice(latitude: Double, longitude: Double) {
        val firestore = FirebaseFirestore.getInstance()
        firestore.collection("kladionice").get()
            .addOnSuccessListener { result ->
                for (document in result) {
                    val geoPoint = document.getGeoPoint("location")
                    geoPoint?.let {
                        val distance = calculateDistance(latitude, longitude, it.latitude, it.longitude)
                        if (distance <= 100 && !notifiedKladionice.contains(document.id)) {
                            sendNearbyKladionicaNotification()
                            notifiedKladionice.add(document.id)
            ...
            .addOnFailureListener { e ->
                Log.e("LocationService", "Greska u fecovanju kladionice", e)
            }
    }
    ...
    private fun sendNearbyKladionicaNotification() {
        val notificationChannelId = "LOCATION_SERVICE_CHANNEL"
        ...
        val notification = NotificationCompat.Builder(this, notificationChannelId)
            .setContentTitle("Kladionica u blizini")
            .setContentText("Nalazite se u blizini neke kladionice!")
            .setSmallIcon(R.drawable.logo)
            .setContentIntent(pendingIntent)
            .build()

        val notificationManager = getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
        notificationManager.notify(NEARBY_KLADIONICA_NOTIFICATION_ID, notification)
    }
    ...
    companion object {
        ...
        private const val NEARBY_KLADIONICA_NOTIFICATION_ID = 2
    }
}